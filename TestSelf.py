...
ki = LINETCR.LINE()
ki.login(token="EpYpHiQAZ4fUfSMbOmb7.sFdOlGKavE/JPdmWVLjKzW.xQogeGQZrePmX5bO7wpPlR7xk3n7n1SDFbK99Z11sw8=")
ki.loginResult()
        ...
        if op.type == 11:
            if op.param2 not in Bots:
                if op.param2 in Bots:
                    ...
                    G.preventJoinByTicket = True
                    ki.updateGroup(G)
                    cl.sendText(op.param1,cl.getContact(op.param2).displayName + "Please Dont Playing Link QR Group Bro 😡😡😡👊👊👊")
                    #cl.kickoutFromGroup(op.param1,[op.param2])
        if op.type == 5:
            if wait["autoAdd"] == True:
                if (wait["message"] in [""," ","\n",None]):
